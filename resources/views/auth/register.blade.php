<x-guest-layout>
    <div class="min-h-screen transition-colors duration-300 dark:bg-gradient-to-br dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 bg-gradient-to-br from-slate-50 via-slate-100 to-slate-50 flex items-center justify-center px-4 py-8">
        <div class="w-full max-w-md">
            ...
            <!-- Register Form Card -->
            <form method="POST" action="{{ route('register') }}" class="transition-colors duration-300 dark:bg-slate-800 bg-white dark:border-slate-700 border-slate-200 border rounded-2xl shadow-2xl overflow-hidden">
                ...
                <div class="transition-colors duration-300 dark:bg-gradient-to-r dark:from-slate-750 dark:to-slate-800 bg-gradient-to-r from-slate-100 to-slate-200 dark:border-slate-700 px-8 py-8 text-center border-b border-slate-200">
                    ...
                    <p class="dark:text-slate-400 text-slate-500 text-sm mt-2">Create your account</p>
                </div>
                ...
                <div class="px-8 py-8 space-y-5">
                    ...
                    <!-- Name -->
                    <div>
                        <label for="name" class="block text-sm font-medium dark:text-slate-300 text-slate-700 mb-2">{{ __('Full Name') }}</label>
                        <input id="name" type="text" name="name" value="{{ old('name') }}" class="w-full rounded-lg transition-colors duration-300 dark:border-slate-600 dark:bg-slate-900 dark:text-white dark:placeholder-slate-400 dark:focus:ring-cyan-500 border-slate-300 bg-white text-slate-900 placeholder-slate-400 focus:ring-cyan-500 border px-4 py-3 focus:outline-none focus:ring-2" placeholder="John Doe" required autofocus autocomplete="name" />
                        @error('name')
                    ...
                    <!-- Login Link -->
                    <div class="text-center text-sm pt-2 dark:border-slate-700 border-t border-slate-200">
