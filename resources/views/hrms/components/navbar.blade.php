<div x-data="{ sidebarOpen: false, scrolled: false }" @scroll.window="scrolled = window.scrollY > 16">
    ...
    {{-- Sidebar --}}
    <aside
        class="fixed inset-y-0 left-0 z-50 w-60 transform border-r border-sky-100 bg-gradient-to-b from-white via-sky-50/60 to-white shadow-xl transition-all duration-300 ease-in-out dark:border-sky-500/10 dark:bg-gradient-to-b dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 lg:translate-x-0"
        ...
        <div class="flex h-full flex-col">
            ...
            <nav class="flex-1 space-y-1 overflow-y-auto px-3 py-3 custom-scrollbar">
                @php
                    ...
                    $navActiveClass = 'border-sky-200 bg-gradient-to-r from-sky-100 to-violet-100 text-indigo-700 shadow-sm dark:border-cyan-400/25 dark:bg-[linear-gradient(135deg,rgba(8,47,73,0.95),rgba(30,41,59,0.92),rgba(76,29,149,0.82))] dark:text-cyan-100 dark:shadow-[0_14px_30px_rgba(14,165,233,0.14)] dark:ring-1 dark:ring-inset dark:ring-white/5';
                    $navIdleClass = 'border-transparent text-slate-500 hover:border-sky-100 hover:bg-sky-50 hover:text-indigo-700 dark:text-slate-400 dark:hover:border-white/10 dark:hover:bg-white/5 dark:hover:text-white';
                @endphp

                <p class="px-3 py-3 text-[9px] font-black uppercase tracking-[0.25em] text-slate-400">Main Console</p>
                @foreach ($navItems as $item)
                    <a href="{{ route($item['route']) }}"
                        class="mb-1 flex items-center gap-2.5 overflow-hidden rounded-xl border px-3 py-2 transition-all duration-200 group {{ request()->routeIs($item['route'].'*') ? $navActiveClass : $navIdleClass }}">
                ...
                <a href="{{ Auth::user()->hasAnyRole(['admin', 'hr_manager']) ? route('policies.index') : route('policies.viewer') }}"
                    class="mb-1 flex items-center gap-2.5 overflow-hidden rounded-xl border px-3 py-2 transition-all duration-200 group {{ request()->routeIs('policies.*') ? $navActiveClass : $navIdleClass }}">
                    <svg class="h-4 w-4 shrink-0 transition-transform group-hover:scale-110" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 0h10.5A2.25 2.25 0 0119.5 12.75v6A2.25 2.25 0 0117.25 21h-10.5A2.25 2.25 0 014.5 18.75v-6A2.25 2.25 0 016.75 10.5z" /></svg>
                    <span class="text-[10px] font-black uppercase tracking-widest">Policies</span>
                </a>

                <p class="mt-6 px-3 py-3 text-[9px] font-black uppercase tracking-[0.25em] text-slate-400">Personnel Hub</p>
                <a href="{{ route('employees.index') }}" class="mb-1 flex items-center gap-2.5 overflow-hidden rounded-xl border px-3 py-2 transition-all duration-200 group {{ request()->routeIs('employees.*') ? $navActiveClass : $navIdleClass }}">
                    <svg class="h-4 w-4 shrink-0 transition-transform group-hover:scale-110" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" /></svg>
                    <span class="text-[10px] font-black uppercase tracking-widest">People</span>
                </a>
                <a href="{{ route('documents.index') }}" class="mb-1 flex items-center gap-2.5 overflow-hidden rounded-xl border px-3 py-2 transition-all duration-200 group {{ request()->routeIs('documents.*') ? $navActiveClass : $navIdleClass }}">
                ...
                @if (Auth::user()->hasAnyRole(['admin', 'hr_manager']))
                    <p class="mt-6 px-3 py-3 text-[9px] font-black uppercase tracking-[0.25em] text-slate-400">Control Center</p>
                    <a href="{{ route('analytics.index') }}" class="mb-1 flex items-center gap-2.5 overflow-hidden rounded-xl border px-3 py-2 transition-all duration-200 group {{ request()->routeIs('analytics.*') ? $navActiveClass : $navIdleClass }}">
                    ...
                    <a href="{{ route('workflows.index') }}" class="mb-1 flex items-center gap-2.5 overflow-hidden rounded-xl border px-3 py-2 transition-all duration-200 group {{ request()->routeIs('workflows.*') ? $navActiveClass : $navIdleClass }}">
                    ...
                    <a href="{{ route('tenant-users.index') }}" class="mb-1 flex items-center gap-2.5 overflow-hidden rounded-xl border px-3 py-2 transition-all duration-200 group {{ request()->routeIs('tenant-users.*') || request()->routeIs('roles.*') || request()->routeIs('users.*') ? $navActiveClass : $navIdleClass }}">
                     ...
                     @if (Auth::user()->hasRole('super_admin'))
                        <a href="{{ route('tenants.index') }}" class="mb-1 flex items-center gap-2.5 overflow-hidden rounded-xl border px-3 py-2 transition-all duration-200 group {{ request()->routeIs('tenants.*') ? $navActiveClass : $navIdleClass }}">
