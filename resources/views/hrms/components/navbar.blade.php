            ...
            <nav class="flex-1 space-y-1 overflow-y-auto px-3 py-3 custom-scrollbar">
                @php
                    $navItems = [
                        ...
                        ['route' => 'departments.index', 'label' => 'Units', 'icon' => 'M2.25 21h19.5m-18-18v18m10.5-18v18m6-13.5V21M6.75 6.75h.75m-.75 3h.75m-.75 3h.75m3-3.75h.75m-.75 3h.75m-.75 3h.75M6.75 21v-3.375c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21M3 3h12m-.75 4.5H21m-3.75 3.75h.008v.008h-.008v-.008zm0 3h.008v.008h-.008v-.008zm0 3h.008v.008h-.008v-.008z'],
                        ['route' => 'leaves.my', 'label' => 'Absence', 'icon' => 'M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5'],
                        ['route' => 'payroll.index', 'label' => 'Payroll Hub', 'icon' => 'M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z'],
                        ['route' => 'attendance.my', 'label' => 'Clock-In', 'icon' => 'M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z'],
                        ['route' => 'shifts.index', 'label' => 'Roster', 'icon' => 'M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5'],
                    ];
                ...
                <p class="px-3 py-2 text-[8px] font-black uppercase tracking-[0.2em] text-slate-400">Core Console</p>
                ...
                <p class="mt-4 px-3 py-2 text-[8px] font-black uppercase tracking-[0.2em] text-slate-400">Governance</p>
                @php
                    $govItems = [
                        ['route' => 'employees.index', 'label' => 'HR Lattice', 'icon' => 'M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z'],
                        ['route' => (Auth::user()->hasAnyRole(['admin', 'hr_manager']) ? 'policies.index' : 'policies.viewer'), 'label' => 'Compliance', 'icon' => 'M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 0h10.5A2.25 2.25 0 0119.5 12.75v6A2.25 2.25 0 0117.25 21h-10.5A2.25 2.25 0 014.5 18.75v-6A2.25 2.25 0 016.75 10.5z'],
                ...
                @if (Auth::user()->hasAnyRole(['admin', 'hr_manager']))
                    ...
                    <a href="{{ route('workflows.index') }}" wire:navigate class="mb-1 flex items-center gap-2.5 overflow-hidden rounded-xl border px-3 py-2 transition-all duration-200 group {{ request()->routeIs('workflows.*') ? $navActiveClass : $navIdleClass }}">
                    ...
                    <a href="{{ route('attendance.index') }}" wire:navigate class="mb-1 flex items-center gap-2.5 overflow-hidden rounded-xl border px-3 py-2 transition-all duration-200 group {{ request()->routeIs('attendance.index') ? $navActiveClass : $navIdleClass }}">
                        ...
                        <span class="text-[10px] font-black uppercase tracking-widest">Tracking</span>
                    </a>
                    <a href="{{ route('settings.index') }}" wire:navigate class="mb-1 flex items-center gap-2.5 overflow-hidden rounded-xl border px-3 py-2 transition-all duration-200 group {{ request()->routeIs('settings.*') ? $navActiveClass : $navIdleClass }}">
