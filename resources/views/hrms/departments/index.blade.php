...
<div class="mb-8 flex items-center justify-between">
    <div>
        ...
        <p class="text-slate-400">Manage organizational structure</p>
    </div>
...
<div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
    @forelse ($departments as $department)
        <div class="rounded-2xl border border-slate-800 bg-slate-900 p-6 hover:border-cyan-500/30">
            <h3 class="text-lg font-semibold">{{ $department->name }}</h3>
            <p class="mt-1 text-sm text-slate-400">Code: {{ $department->code }}</p>
            <p class="mt-1 text-sm text-slate-400">Lead: {{ $department->lead_name }}</p>
