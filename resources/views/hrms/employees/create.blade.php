    ...
    <form method="POST" action="{{ route('employees.store') }}" class="space-y-4">
        ...
        <div class="grid gap-4 md:grid-cols-2">
            ...
            <div>
                ...
                <input type="date" name="joined_on" class="mt-1 w-full rounded-lg border border-slate-300 bg-slate-50 dark:border-slate-700 dark:bg-slate-950 px-3 py-2 @error('joined_on') border-red-500 @enderror" value="{{ old('joined_on') }}" required>
                @error('joined_on')
                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                @enderror
            </div>
