...
<div class="mb-8">
    ...
    <p class="text-slate-600 dark:text-slate-400">Create a new employee record</p>
</div>
...
<div class="max-w-4xl rounded-2xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900 p-6">
    <form method="POST" action="{{ route('employees.store') }}" class="space-y-6">
        ...
        <div>
            ...
            <div class="mt-3 grid gap-4 md:grid-cols-2">
                <div>
                    ...
                    <input type="text" name="full_name" class="mt-1 w-full rounded-lg border border-slate-300 bg-slate-50 dark:border-slate-700 dark:bg-slate-950 px-3 py-2 @error('full_name') border-red-500 @enderror" value="{{ old('full_name') }}" required>
                    @error('full_name')<p class="mt-1 text-sm text-red-400">{{ $message }}</p>@enderror
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300">Email</label>
                    <input type="email" name="email" class="mt-1 w-full rounded-lg border border-slate-300 bg-slate-50 dark:border-slate-700 dark:bg-slate-950 px-3 py-2 @error('email') border-red-500 @enderror" value="{{ old('email') }}" required>
                    @error('email')<p class="mt-1 text-sm text-red-400">{{ $message }}</p>@enderror
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300">Phone</label>
                    <input type="tel" name="phone" class="mt-1 w-full rounded-lg border border-slate-300 bg-slate-50 dark:border-slate-700 dark:bg-slate-950 px-3 py-2 @error('phone') border-red-500 @enderror" value="{{ old('phone') }}" required>
                ...
                <div>
                    ...
                    <input type="text" name="job_title" class="mt-1 w-full rounded-lg border border-slate-300 bg-slate-50 dark:border-slate-700 dark:bg-slate-950 px-3 py-2 @error('job_title') border-red-500 @enderror" value="{{ old('job_title') }}" required>
                ...
                <div>
                    ...
                    <select name="role_id" class="mt-1 w-full rounded-lg border border-slate-300 bg-slate-50 dark:border-slate-700 dark:bg-slate-950 px-3 py-2">
                        ...
                        @foreach ($roles as $role)
                            <option value="{{ $role->id }}" @selected(old('role_id') == $role->id || $role->name === 'employee')>{{ $role->display_name ?? ucfirst($role->name) }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number" step="0.01" min="0" name="salary" class="mt-1 w-full rounded-lg border border-slate-300 bg-slate-50 dark:border-slate-700 dark:bg-slate-950 px-3 py-2 @error('salary') border-red-500 @enderror" value="{{ old('salary') }}" required>
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300">Join Date</label>
                                <input type="date" name="joined_on" class="transition-colors duration-300 mt-1 w-full rounded-lg border border-slate-300 bg-white text-slate-900 dark:border-slate-700 dark:bg-slate-950 dark:text-white px-3 py-2 appearance-auto @error('joined_on') border-red-500 @enderror" value="{{ old('joined_on') }}" required>
        ...
        <div>
            ...
            <div class="mt-3 grid gap-4 md:grid-cols-2">
                <div>
                    ...
                    <input type="text" name="country" list="country-options" maxlength="3" class="mt-1 w-full rounded-lg border border-slate-300 bg-slate-50 dark:border-slate-700 dark:bg-slate-950 px-3 py-2 @error('country') border-red-500 @enderror" value="{{ old('country', 'IN') }}" required>
                ...
                <div>
                    ...
                    <input type="text" name="state" list="state-options" maxlength="3" class="mt-1 w-full rounded-lg border border-slate-300 bg-slate-50 dark:border-slate-700 dark:bg-slate-950 px-3 py-2 @error('state') border-red-500 @enderror" value="{{ old('state') }}" required>
                ...
                <div>
                    ...
                    <input type="text" name="city" class="mt-1 w-full rounded-lg border border-slate-300 bg-slate-50 dark:border-slate-700 dark:bg-slate-950 px-3 py-2 @error('city') border-red-500 @enderror" value="{{ old('city') }}" required>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="address" rows="3" class="mt-1 w-full rounded-lg border border-slate-300 bg-slate-50 dark:border-slate-700 dark:bg-slate-950 px-3 py-2 @error('address') border-red-500 @enderror" required>{{ old('address') }}</textarea>
        ...
        <div>
            ...
            <div class="mt-3 grid gap-4 md:grid-cols-2">
                <div class="md:col-span-2">
                    ...
                    <textarea name="hobbies" rows="2" class="mt-1 w-full rounded-lg border border-slate-300 bg-slate-50 dark:border-slate-700 dark:bg-slate-950 px-3 py-2 @error('hobbies') border-red-500 @enderror">{{ old('hobbies') }}</textarea>
                </div>
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300">Likes</label>
                    <textarea name="likes" rows="2" class="mt-1 w-full rounded-lg border border-slate-300 bg-slate-50 dark:border-slate-700 dark:bg-slate-950 px-3 py-2 @error('likes') border-red-500 @enderror">{{ old('likes') }}</textarea>
                </div>
                <div>
                    ...
                    <select name="food_preference" class="mt-1 w-full rounded-lg border border-slate-300 bg-slate-50 dark:border-slate-700 dark:bg-slate-950 px-3 py-2 @error('food_preference') border-red-500 @enderror">
                        ...
                        <option value="veg" @selected(old('food_preference') === 'veg')>Veg</option>
                        <option value="non-veg" @selected(old('food_preference') === 'non-veg')>Non-Veg</option>
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300">Health Issues</label>
                    <input type="text" name="health_issues" class="mt-1 w-full rounded-lg border border-slate-300 bg-slate-50 dark:border-slate-700 dark:bg-slate-950 px-3 py-2 @error('health_issues') border-red-500 @enderror" value="{{ old('health_issues') }}">
                </div>
            </div>
        </div>
        ...
        <div class="flex gap-3 pt-2">
            ...
            <a href="{{ route('employees.index') }}" class="rounded-lg border border-slate-300 px-6 py-2 font-semibold text-slate-700 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800 hover:bg-slate-100">Cancel</a>
        </div>
    </form>
</div>
@endsection
