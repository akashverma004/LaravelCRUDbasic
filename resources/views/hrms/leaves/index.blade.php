    ...
    <div class="relative overflow-hidden rounded-2xl bg-white px-8 py-8 shadow-sm border border-slate-200 dark:bg-slate-900/50 dark:border-slate-800">
        ...
        <div class="relative flex flex-col items-start justify-between gap-6 lg:flex-row lg:items-center">
            <div>
                ...
                <div class="mt-4 flex flex-wrap items-center gap-4 text-xs font-semibold text-slate-600 dark:text-slate-400">
            ...
            <a href="{{ route('leaves.create') }}" class="inline-flex items-center gap-2 rounded-xl bg-cyan-500 px-5 py-2.5 text-sm font-semibold text-slate-950 shadow-sm transition-colors hover:bg-cyan-400">
                <span>Add Leave</span>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/50">
        <form method="GET" action="{{ route('leaves.index') }}" class="flex flex-wrap items-end gap-6">
            ...
            <div class="flex-1 min-w-[280px] space-y-2">
                ...
                <div class="relative">
                    <div class="absolute inset-y-0 left-4 flex items-center pointer-events-none">
                        <svg class="h-4 w-4 text-slate-400" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                    </div>
                    <input
                        id="q" name="q" value="{{ $filters['q'] ?? '' }}"
                        placeholder="Search employees..."
                        class="w-full rounded-xl border border-slate-200 bg-transparent pl-10 pr-4 py-2.5 text-sm font-medium text-slate-900 placeholder:text-slate-400 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 dark:border-slate-700 dark:text-white"
                    >
                </div>
            </div>
            <div class="w-64 space-y-2">
                <label class="block text-xs font-semibold text-slate-700 dark:text-slate-400">Department</label>
                <div class="relative">
                     <select id="department_id" name="department_id" class="w-full rounded-xl border border-slate-200 bg-transparent px-4 py-2.5 text-sm font-medium text-slate-900 appearance-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 dark:border-slate-700 dark:text-white">
            ...
            <div class="flex gap-2">
                <button type="submit" class="rounded-xl bg-slate-900 px-6 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-slate-800 dark:bg-white dark:text-slate-950 dark:hover:bg-slate-100">Filter</button>
                <a href="{{ route('leaves.index', ['month' => $monthStart->format('Y-m')]) }}" class="rounded-xl border border-slate-200 bg-white px-6 py-2.5 text-sm font-semibold text-slate-700 transition-colors hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-800">Clear</a>
            </div>
        ...
        <div class="overflow-x-auto custom-scrollbar border-t border-slate-100 dark:border-slate-800">
            <div class="min-w-[1200px]">
                ...
                <div class="grid border-b border-slate-200 dark:border-slate-800" style="grid-template-columns: 320px repeat({{ $calendarDays->count() }}, minmax(40px, 1fr));">
                    <div class="flex items-center justify-between border-r border-slate-200 bg-slate-50 px-6 py-4 dark:border-slate-800 dark:bg-slate-950/30 sticky left-0 z-20">
                        <div>
                            <h2 class="text-lg font-bold text-slate-900 dark:text-white">{{ $monthStart->format('F Y') }}</h2>
                        </div>
                        <div class="flex items-center gap-2">
