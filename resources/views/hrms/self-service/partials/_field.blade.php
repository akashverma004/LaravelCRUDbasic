            ...
            @if($readonly)
                <div class="relative group">
                    <div class="flex items-center rounded-xl border border-slate-100 bg-slate-50 px-3 py-1.5 dark:border-slate-800 dark:bg-slate-950/40 transition-all">
                        <span class="text-xs font-semibold text-slate-600 dark:text-slate-300 truncate" x-text="(() => { const value = employee.{{ $field }}; if (!value) return '-'; if ('{{ $type }}' === 'date') { const date = new Date(value); return Number.isNaN(date.getTime()) ? value : date.toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }); } return value; })()"></span>
                        <svg class="h-3 w-3 ml-auto text-slate-300 group-hover:text-slate-500 transition-colors" fill="none" viewBox="0 0 24 24" stroke-width="3" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z" /></svg>
                    </div>
                </div>
            @else
                <template x-if="!editing">
                    <div class="flex items-center rounded-xl border border-slate-100 bg-slate-50 px-3 py-1.5 dark:border-slate-800 dark:bg-slate-950/40 transition-all hover:border-cyan-400/30">
                        <span class="text-xs font-semibold text-slate-900 dark:text-white truncate" x-text="(() => { const value = employee.{{ $field }}; if (!value) return '-'; if ('{{ $type }}' === 'date') { const date = new Date(value); return Number.isNaN(date.getTime()) ? value : date.toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }); } return value; })()"></span>
                    </div>
                </template>
                <template x-if="editing">
                    <div class="relative group">
                        <input type="{{ $type }}" x-model="form.{{ $field }}" 
                            ...
                            class="w-full rounded-xl border bg-slate-50 px-3 py-1.5 text-xs font-bold text-slate-900 dark:bg-slate-950 dark:text-white transition-all focus:ring-4 focus:ring-cyan-500/10 placeholder:text-slate-300">
