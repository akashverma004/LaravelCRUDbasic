            ...
            @if($readonly)
                <div class="flex items-center rounded-xl border border-slate-100 bg-slate-50 px-4 py-2.5 dark:border-slate-800 dark:bg-slate-950/40 transition-all">
                    <span class="text-xs font-bold text-slate-400 uppercase tracking-tight"
                        x-text="(() => { const v = employee.{{ $field }}; return v ? String(v).replace(/[_-]/g, ' ').replace(/\b\w/g, c => c.toUpperCase()) : 'REDACTED'; })()"></span>
            ...
            @else
                <template x-if="!editing">
                    <div class="flex items-center rounded-xl border border-slate-100 bg-slate-50 px-4 py-2.5 dark:border-slate-800 dark:bg-slate-950/40 transition-all hover:border-cyan-400/30">
                        <span class="text-xs font-black text-slate-900 dark:text-white uppercase tracking-tight"
                            x-text="(() => { const v = employee.{{ $field }}; return v ? String(v).replace(/[_-]/g, ' ').replace(/\b\w/g, c => c.toUpperCase()) : 'SELECT_DATA'; })()"></span>
                    </div>
                </template>
                <template x-if="editing">
                    <div class="relative group">
                        <select x-model="form.{{ $field }}" 
                            ...
                            class="w-full rounded-xl border bg-slate-50 px-4 py-2.5 text-xs font-bold text-slate-900 dark:bg-slate-950 dark:text-white transition-all focus:ring-4 focus:ring-cyan-500/10 appearance-none">
                            <option value="">Select Option...</option>
                            @foreach($options as $value => $text)
                                <option value="{{ $value }}">{{ $text }}</option>
                            @endforeach
                        </select>
                        <div class="pointer-events-none absolute right-4 inset-y-0 flex items-center">
                            <svg class="h-4 w-4 text-slate-400" fill="none" viewBox="0 0 24 24" stroke-width="3" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" /></svg>
                        </div>
