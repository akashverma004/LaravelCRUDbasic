...
<div x-data="selfServiceProfile()" x-init="init()" class="space-y-6 relative">
    ...
    {{-- Profile Hero --}}
    <div class="relative overflow-hidden rounded-2xl bg-white px-5 py-6 shadow-sm border border-slate-200 dark:border-white/5 dark:bg-slate-900">
        ...
        <div class="relative flex flex-col items-center gap-6 lg:flex-row lg:items-center">
            {{-- Photo Container --}}
            <div class="group relative">
                <div class="h-24 w-24 overflow-hidden rounded-full border-4 border-slate-50 shadow-md transition-all group-hover:scale-[1.02] dark:border-slate-800">
                    ...
                    <template x-if="!photoUrl">
                        <div class="flex h-full w-full items-center justify-center bg-slate-100 dark:bg-slate-800">
                            <span class="text-3xl font-bold text-slate-400 dark:text-slate-500" x-text="employee.full_name ? employee.full_name.substring(0, 1).toUpperCase() : '?'"></span>
                        </div>
                    </template>
                </div>
            ...
            <div class="flex-1 text-center lg:text-left">
                <div class="flex flex-wrap items-center justify-center gap-3 lg:justify-start">
                    <h2 class="text-2xl font-bold tracking-tight text-slate-900 dark:text-white" x-text="employee.full_name"></h2>
                    <span class="rounded-full bg-cyan-50 px-2 py-0.5 text-[9px] font-semibold uppercase tracking-wider text-cyan-700 dark:bg-cyan-500/10 dark:text-cyan-400" x-text="employee.status"></span>
                </div>
                <p class="mt-1 text-xs font-medium text-slate-500 dark:text-slate-400" x-text="(employee.job_title || 'Employee') + ' • ' + (employee.department || 'Not Assigned')"></p>
                <div class="mt-4 flex flex-wrap items-center justify-center gap-4 lg:justify-start">
                    <div class="flex items-center gap-1.5 text-slate-500 dark:text-slate-400">
                        ...
                        <span class="text-[10px] font-medium uppercase tracking-wide" x-text="'Joined: ' + (employee.joined_on || 'Unknown')"></span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="space-y-6">
            ...
            <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/40">
                <div class="mb-4 flex items-center justify-between">
                    <h3 class="text-[9px] font-black uppercase tracking-widest text-slate-400">Skills</h3>
                    <button @click="showSkillForm = !showSkillForm" class="flex h-6 w-6 items-center justify-center rounded-lg bg-slate-100 text-slate-600 hover:bg-slate-200 transition-colors dark:bg-white/5 dark:text-slate-300 dark:hover:bg-white/10">
                ...
                <div x-show="showSkillForm" x-transition class="mb-4 rounded-xl bg-slate-50 p-3 border border-slate-200 space-y-3 dark:bg-slate-950 dark:border-white/5">
                    <div>
                         <label class="block mb-1 text-[9px] font-black uppercase tracking-widest text-slate-400 ml-1">Skill Name</label>
                         <input type="text" x-model="skillForm.name" placeholder="E.g., Design" class="w-full rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-bold text-slate-900 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 dark:border-white/10 dark:bg-slate-900 dark:text-white">
                    </div>
                    <div>
                         <label class="block mb-1 text-[9px] font-black uppercase tracking-widest text-slate-400 ml-1">Proficiency</label>
                         <select x-model="skillForm.proficiency" class="w-full rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-bold text-slate-900 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 appearance-none dark:border-white/10 dark:bg-slate-900 dark:text-white">
                    ...
                    <div class="flex gap-2">
                        <button @click="addSkill()" class="flex-1 rounded-lg bg-cyan-500 py-1.5 text-[10px] uppercase tracking-widest font-black text-white hover:bg-cyan-600 transition-colors">Add</button>
                        <button @click="showSkillForm = false" class="rounded-lg border border-slate-200 px-3 py-1.5 text-[10px] uppercase tracking-widest font-black text-slate-500 hover:bg-slate-100 transition-colors dark:border-white/10 dark:text-slate-400 dark:hover:bg-white/5">Cancel</button>
                    </div>
                </div>
                ...
                <div class="flex flex-wrap gap-1.5">
                    <template x-for="skill in employee.skills || []" :key="skill.id">
                        <div class="group flex items-center gap-1.5 rounded-lg border border-slate-200 bg-slate-50 px-2 py-1 dark:border-white/5 dark:bg-slate-950 transition-colors hover:border-cyan-300 dark:hover:border-cyan-700">
                            ...
                            <span class="text-[10px] font-bold text-slate-700 dark:text-slate-300 uppercase tracking-tight" x-text="skill.name"></span>
            ...
            {{-- Job Details --}}
            <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/40">
                <h3 class="mb-4 text-[9px] font-black uppercase tracking-widest text-slate-400">Job Details</h3>
                <div class="space-y-4">
                    <template x-for="item in [
                        ...
                        <div class="flex items-center gap-3">
                            <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-slate-50 text-slate-400 dark:bg-white/5 dark:text-slate-500">
                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" :d="item.icon" /></svg>
                            </div>
                            <div class="min-w-0">
                                <p class="text-[8px] font-black uppercase tracking-widest text-slate-400" x-text="item.label"></p>
                                <p class="mt-0.5 text-[11px] font-bold text-slate-900 truncate dark:text-white capitalize tracking-tight" x-text="item.value || 'Not Assigned'"></p>
                            </div>
                        </div>
                    </template>
                </div>
            ...
            <div class="flex gap-1.5 overflow-x-auto rounded-xl border border-slate-200 bg-white p-1.5 shadow-sm dark:border-slate-800 dark:bg-slate-900/50 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                <template x-for="t in tabs" :key="t.id">
                    <button @click="activeTab = t.id" 
                        class="whitespace-nowrap rounded-lg px-3 py-1.5 text-[10px] font-black uppercase tracking-widest transition-colors"
                        :class="activeTab === t.id ? 'bg-slate-100 text-slate-900 dark:bg-slate-800 dark:text-white' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-800/50 dark:hover:text-white'"
