...
@section('content')
<div x-data="selfServiceProfile()" x-init="init()" class="space-y-8">
    ...
    {{-- Profile Hero --}}
    <div class="relative overflow-hidden rounded-2xl bg-white px-8 py-10 shadow-sm border border-slate-200 dark:border-white/5 dark:bg-slate-900">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-8">
            {{-- Tabs --}}
            <div class="flex gap-2 overflow-x-auto rounded-2xl border border-slate-200 bg-white p-2 shadow-sm dark:border-slate-800 dark:bg-slate-900/40 custom-scrollbar">
                <template x-for="t in tabs" :key="t.id">
                    <button @click="activeTab = t.id" 
                        class="flex shrink-0 items-center justify-center rounded-xl px-4 py-2 text-xs font-semibold transition-colors"
                        :class="activeTab === t.id ? 'bg-slate-900 text-white dark:bg-slate-800 dark:text-white' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-white/5 dark:hover:text-white'"
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8 dark:border-slate-800 dark:bg-slate-900/40">
                <div class="mb-8 flex flex-col sm:flex-row items-center justify-between gap-4">
                    ...
                    <div x-show="['personal', 'emergency', 'identity', 'bank', 'preferences'].includes(activeTab)">
                        <button x-show="!editing" @click="startEditing()" class="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2 text-xs font-semibold text-slate-700 border border-slate-200 shadow-sm hover:bg-slate-50 transition-colors dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700">
                        ...
                        <div x-show="editing" class="flex items-center gap-4" style="display: none;">
                            <span class="text-[10px] font-bold text-cyan-600 animate-pulse">EDIT MODE ACTIVE</span>
                            <div class="flex gap-2">
                                <button @click="cancelEditing()" class="rounded-xl border border-slate-200 px-4 py-2 text-xs font-medium text-slate-600 bg-white hover:bg-slate-50 transition-colors dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700">Cancel</button>
                                <button @click="saveProfile()" :disabled="saving" class="inline-flex items-center gap-2 rounded-xl bg-cyan-500 px-4 py-2 text-xs font-bold text-white shadow-sm transition-colors hover:bg-cyan-600 disabled:opacity-50">
