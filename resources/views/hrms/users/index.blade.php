...
>
        ...
        <div class="relative flex flex-col items-start justify-between gap-6 lg:flex-row lg:items-center">
            <div>
                <h1 class="text-3xl font-bold tracking-tight text-slate-900 dark:text-white lg:text-4xl">
                    User <span class="text-cyan-600 dark:text-cyan-400">Management</span>
                </h1>
                    ...
                    <template x-for="user in users" :key="user.id">
                        <tr class="group hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-right">
                                <button @click="openModal(user)" class="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-600 shadow-sm transition-colors hover:bg-slate-50 hover:text-cyan-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700 dark:hover:text-cyan-400">
                                    Manage Roles
                                </button>
                            </td>
                        </tr>
    ...
    <div x-show="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/50 backdrop-blur-sm" style="display: none;" x-transition>
        <div @click.away="closeModal()" class="w-full max-w-lg rounded-2xl bg-white shadow-xl dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex flex-col max-h-[90vh]">
            ...
            <div class="flex flex-shrink-0 items-center justify-end gap-3 border-t border-slate-100 bg-slate-50 px-6 py-4 dark:border-slate-800 dark:bg-slate-900/50">
                ...
                <button @click="saveRoles()" :disabled="saving" class="inline-flex items-center gap-2 rounded-xl bg-cyan-500 px-6 py-2 text-sm font-bold text-white shadow-sm transition-colors hover:bg-cyan-600 disabled:opacity-50">
