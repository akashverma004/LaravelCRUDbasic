<div class="space-y-5 pb-8">
    ...
    <div class="relative overflow-hidden rounded-xl bg-white px-4 py-4 shadow-sm border border-slate-200 dark:bg-slate-900/50 dark:border-white/5">
        ...
        <div class="relative flex flex-col items-start justify-between gap-3 lg:flex-row lg:items-center">
            <div>
                ...
                <p class="mt-2 text-[8px] font-bold text-slate-500 uppercase tracking-widest">
            ...
            <div class="flex flex-wrap items-center gap-3">
                <div class="flex items-center rounded-xl bg-slate-50 p-1 dark:bg-white/5 border border-slate-100 dark:border-white/5 shadow-inner">
                    ...
                    <button wire:click="$set('weekStart', '{{ now()->startOfWeek()->toDateString() }}')" class="px-4 text-[8px] font-black uppercase tracking-widest text-slate-600 hover:text-indigo-600 dark:text-slate-400 dark:hover:text-indigo-400">Today</button>
                ...
                @if($isAdmin)
                    <button wire:click="$set('showShiftModal', true)" class="inline-flex h-11 items-center gap-2 rounded-xl bg-slate-900 px-6 text-[8px] font-black uppercase tracking-widest text-white shadow-lg transition-all hover:bg-indigo-600 active:scale-95 dark:bg-white/5 dark:text-indigo-400">
            ...
            <table class="w-full text-left border-collapse min-w-[1000px]">
                <thead>
                    <tr class="bg-slate-50/50 dark:bg-white/[0.02]">
                        <th class="sticky left-0 z-10 bg-slate-50/50 dark:bg-slate-950 px-4 py-3 border-r border-slate-100 dark:border-white/5 w-[220px]">
                            <span class="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400">Staff Members</span>
                        </th>
                        @foreach($days as $day)
                            <th class="px-3 py-3 border-r border-slate-100 dark:border-white/5 text-center {{ $day->isToday() ? 'bg-indigo-50/30 dark:bg-indigo-500/5' : '' }}">
                                <p class="text-[8px] font-black uppercase tracking-widest text-slate-400">{{ $day->format('D') }}</p>
                                <p class="text-[11px] font-black text-slate-900 dark:text-white mt-0.5 {{ $day->isToday() ? 'text-indigo-600 dark:text-indigo-400' : '' }}">{{ $day->format('d M') }}</p>
                            </th>
                        @endforeach
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100 dark:divide-white/5">
                    @foreach($employees as $employee)
                        <tr class="group hover:bg-slate-50/50 dark:hover:bg-white/[0.01]">
                            <td class="sticky left-0 z-10 bg-white dark:bg-slate-900 border-r border-slate-100 dark:border-white/5 px-4 py-3 group-hover:bg-slate-50/50 dark:group-hover:bg-white/[0.01]">
                                <div class="flex items-center gap-2.5">
                                    <div class="h-8 w-8 rounded-xl bg-slate-100 flex items-center justify-center font-black text-[8px] text-slate-500 dark:bg-white/5 dark:text-indigo-400 shrink-0">
                                        {{ substr($employee->full_name, 0, 1) }}
                                    </div>
                                    <div class="min-w-0">
                                        <p class="truncate text-[10px] font-black text-slate-900 dark:text-white uppercase tracking-tight">{{ $employee->full_name }}</p>
                                        <p class="truncate text-[8px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">{{ $employee->job_title }}</p>
                                    </div>
                                </div>
                            </td>
                            @foreach($days as $day)
                                ...
                                @endphp
                                <td class="p-1.5 border-r border-slate-100 dark:border-white/5 {{ $day->isToday() ? 'bg-indigo-50/10 dark:bg-indigo-500/[0.02]' : '' }} relative min-h-[60px]">
                                    @if($assignment)
                                        <div class="group/item relative rounded-lg border p-1.5 h-full flex flex-col justify-between transition-all hover:shadow-sm" 
                                            ...
                                            <div>
                                                ...
                                                <p class="text-[9px] font-bold text-slate-600 dark:text-slate-400 tabular-nums leading-none">
                                                    {{ \Carbon\Carbon::parse($assignment->shift->start_time)->format('H:i') }} - {{ \Carbon\Carbon::parse($assignment->shift->end_time)->format('H:i') }}
                                                </p>
                                            </div>
                                            <div class="h-1 w-full rounded-full mt-1.5" style="background-color: {{ $assignment->shift->color }}50;"></div>
                                        </div>
                                    @elseif($isAdmin)
                                        <button wire:click="openAssignModal({{ $employee->id }}, '{{ $dateStr }}')"
                                                class="w-full h-full min-h-[50px] flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all rounded-lg border-2 border-dashed border-slate-100 hover:border-indigo-300 hover:bg-indigo-50/30 dark:border-white/5 dark:hover:bg-indigo-500/5">
                                            <svg class="h-4 w-4 text-slate-300 group-hover:text-indigo-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" /></svg>
                                        </button>
                                    @endif
                                </td>
                            @endforeach
    ...
    @if($showAssignModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative w-full max-w-md rounded-xl bg-white shadow-2xl dark:bg-slate-900 border border-slate-200 dark:border-white/10 overflow-hidden">
                <div class="border-b border-slate-100 p-3 dark:border-white/5">
                    <h2 class="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tight">Assign <span class="text-indigo-500">Shift</span></h2>
                ...
                <div class="p-3 space-y-4">
                    <div class="space-y-1.5">
                        <label class="text-[8px] font-black uppercase tracking-widest text-slate-500">Select Shift Template</label>
                ...
                <div class="border-t border-slate-100 bg-slate-50 p-3 dark:border-white/5 dark:bg-white/5 flex justify-end gap-3">
                    <button wire:click="$set('showAssignModal', false)" class="rounded-xl border border-slate-200 bg-white px-6 py-2.5 text-[8px] font-black uppercase tracking-widest text-slate-700 transition hover:bg-slate-50">Cancel</button>
                    <button wire:click="assignShift" class="rounded-xl bg-slate-900 px-8 py-2.5 text-[8px] font-black uppercase tracking-widest text-white shadow-xl transition-all hover:bg-indigo-600 active:scale-95">Confirm Assignment</button>
                </div>
            </div>
        </div>
    @endif
    ...
    @if($showShiftModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative w-full max-w-xl rounded-xl bg-white shadow-2xl dark:bg-slate-900 border border-slate-200 dark:border-white/10 overflow-hidden">
                <div class="border-b border-slate-100 p-3 dark:border-white/5 flex items-center justify-between">
                    <h2 class="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tight">Shift <span class="text-indigo-500">Templates</span></h2>
                ...
                <div class="p-3 grid grid-cols-1 md:grid-cols-2 gap-4">
                    {{-- Create Form --}}
                    <div class="space-y-4">
                        <h3 class="text-[8px] font-black uppercase tracking-widest text-slate-400">New Template</h3>
                        <div class="space-y-4">
                            <div class="space-y-1.5">
                                <label class="text-[9px] font-black uppercase text-slate-500">Shift Name</label>
                                <input wire:model="shiftName" type="text" placeholder="e.g. Morning Shift" class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-[9px] font-black text-slate-900 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 dark:border-white/5 dark:bg-white/5 dark:text-white">
                            </div>
                            <div class="grid grid-cols-2 gap-4">
                                <div class="space-y-1.5">
                                    <label class="text-[9px] font-black uppercase text-slate-500">Start Time</label>
                                    <input wire:model="startTime" type="time" class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-[9px] font-black text-slate-900 dark:border-white/5 dark:bg-white/5 dark:text-white">
                                </div>
                                <div class="space-y-1.5">
                                    <label class="text-[9px] font-black uppercase text-slate-500">End Time</label>
                                    <input wire:model="endTime" type="time" class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-[9px] font-black text-slate-900 dark:border-white/5 dark:bg-white/5 dark:text-white">
                                </div>
                            </div>
                            <div class="space-y-1.5">
                            ...
                            <button wire:click="createShift" class="w-full rounded-xl bg-slate-900 py-3 text-[8px] font-black uppercase tracking-widest text-white transition-all hover:bg-slate-800 active:scale-95">Add Template</button>
                        </div>
                    </div>
                    {{-- Templates List --}}
                    <div class="space-y-4">
                        <h3 class="text-[8px] font-black uppercase tracking-widest text-slate-400">Existing Templates</h3>
                            ...
                            @foreach($shifts as $shift)
                                <div class="flex items-center justify-between p-3 rounded-xl border border-slate-200 dark:border-white/5 bg-slate-50/50 dark:bg-white/5">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p class="text-[8px] font-black text-slate-900 dark:text-white uppercase">{{ $shift->name }}</p>
