<div class="space-y-8 pb-12">
    {{-- Header --}}
    <div class="relative overflow-hidden rounded-[1.5rem] bg-white px-10 py-10 shadow-sm border border-slate-200 dark:bg-slate-900/50 dark:border-white/5">
        ...
        <div class="relative flex flex-col items-start justify-between gap-3 lg:flex-row lg:items-center">
            <div class="flex-1">
                ...
                <p class="mt-3 text-[11px] font-bold text-slate-500 uppercase tracking-widest leading-loose max-w-lg">
            ...
            <div class="flex gap-4 p-1.5 rounded-xl bg-slate-100 dark:bg-white/5 shadow-inner">
    ...
    @if($activeTab === 'policies')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            @foreach($policies as $policy)
                <div class="group relative flex flex-col rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm dark:border-white/5 dark:bg-slate-900 transition-all hover:shadow-xl hover:border-cyan-400/30">
                    <div class="flex items-start justify-between mb-6">
                        <div class="h-14 w-14 flex items-center justify-center rounded-xl bg-cyan-50 font-black text-[15px] text-cyan-600 shadow-inner dark:bg-cyan-500/10 dark:text-cyan-400 transition-transform group-hover:scale-110 uppercase">
                            {{ $policy->country_code }}
                        </div>
                    ...
                    <h4 class="text-base font-black text-slate-900 dark:text-white uppercase tracking-tight truncate">{{ $policy->name }}</h4>
            ...
            <button wire:click="openPolicyModal()" class="group relative cursor-pointer flex flex-col items-center justify-center rounded-[1.5rem] border-4 border-dashed border-slate-100 p-12 text-center transition-all hover:bg-slate-50 hover:border-cyan-400 dark:border-white/5 dark:hover:bg-white/2">
                <div class="h-16 w-16 flex items-center justify-center rounded-xl bg-white shadow-sm border border-slate-100 text-slate-300 dark:bg-slate-900 dark:border-white/10 group-hover:bg-cyan-600 group-hover:text-white group-hover:border-cyan-600 transition-all">
    ...
    @else
        ...
        <div class="flex flex-col lg:flex-row gap-3">
            ...
            <aside class="w-full lg:w-96 shrink-0 space-y-4">
                <div class="rounded-[2rem] bg-slate-100 p-4 dark:bg-white/5 border border-slate-200 dark:border-white/10">
                    ...
                    @foreach(App\Models\HolidayPolicy::all() as $p)
                        <button wire:click="$set('selectedPolicyId', {{ $p->id }})" class="w-full px-5 py-4 rounded-xl mb-2 flex items-center justify-between gap-4 transition-all {{ $selectedPolicyId == $p->id ? 'bg-white shadow-xl text-cyan-600 dark:bg-slate-900 dark:text-cyan-400' : 'text-slate-400 hover:bg-white/40 dark:hover:bg-white/2 hover:text-slate-600' }}">
            ...
            <main class="flex-1 space-y-6">
                @if($selectedPolicy)
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                             <h3 class="text-lg font-black text-slate-900 dark:text-white uppercase tracking-tight">{{ $selectedPolicy->name }} <span class="text-cyan-500">Epochs</span></h3>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        @foreach($selectedPolicy->holidayDates as $date)
                            <div class="group flex items-center justify-between p-4 rounded-xl border border-slate-100 bg-white dark:border-white/5 dark:bg-slate-900/50 hover:shadow-lg transition-all">
                                <div class="flex items-center gap-3">
                                    <div class="flex flex-col items-center justify-center h-14 w-14 rounded-xl bg-slate-50 border border-slate-100 dark:bg-white/5 dark:border-white/10 shadow-inner shrink-0 group-hover:bg-cyan-50 group-hover:border-cyan-100 transition-colors">
                ...
                @else
                    <div class="py-32 rounded-[2rem] bg-slate-50 border-4 border-dashed border-slate-100 flex flex-col items-center text-center dark:bg-white/2 dark:border-white/5">
                        ...
                        <h4 class="text-base font-black text-slate-400 uppercase tracking-widest">Select a jurisdictional node to inspect temporal epochs.</h4>
                    </div>
                @endif
            </main>
        </div>
    ...
    @if($showPolicyModal)
        <div class="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <div class="relative w-full max-w-lg rounded-[2rem] bg-white shadow-2xl dark:bg-slate-950 border border-slate-200 dark:border-white/10 flex flex-col animate-in fade-in zoom-in duration-300">
                <div class="flex items-center justify-between px-10 py-8 border-b border-slate-50 dark:border-white/5">
                    <h2 class="text-lg font-black text-slate-900 dark:text-white uppercase tracking-tight">Temporal <span class="text-cyan-500">Node Provisioning</span></h2>
                ...
                <form wire:submit="savePolicy" class="p-10 space-y-8">
                    <div class="grid grid-cols-2 gap-3">
                        <div class="space-y-1.5">
                            <label class="text-[10px] font-black uppercase text-slate-500 tracking-widest ml-1 text-[10px]">Node Descriptor</label>
                            <input wire:model="name" type="text" placeholder="Organizational Sector Policy..." class="w-full rounded-xl border border-slate-200 bg-slate-50 px-6 py-4 text-[10px] font-black text-slate-900 dark:border-white/5 dark:bg-white/5 dark:text-white uppercase tracking-widest focus:ring-0 focus:border-cyan-400">
                        </div>
                        <div class="space-y-1.5">
                            <label class="text-[10px] font-black uppercase text-slate-500 tracking-widest ml-1 text-[10px]">Sector Slug</label>
                            <input wire:model="code" type="text" placeholder="HOLIDAY_DEFAULT..." class="w-full rounded-xl border border-slate-200 bg-slate-50 px-6 py-4 text-[10px] font-black text-slate-900 dark:border-white/5 dark:bg-white/5 dark:text-white uppercase tracking-widest disabled:opacity-50" {{ $editingPolicyId ? 'disabled' : '' }}>
                        </div>
                    </div>
                    <div class="grid grid-cols-2 gap-3">
                        <div class="space-y-1.5">
                            <label class="text-[10px] font-black uppercase text-slate-500 tracking-widest ml-1 text-[10px]">Country Sector</label>
                            <select wire:model="countryCode" class="w-full rounded-xl border border-slate-200 bg-slate-50 px-6 py-4 text-[10px] font-black text-slate-900 dark:border-white/5 dark:bg-white/5 dark:text-white uppercase tracking-widest">
                        ...
                        <div class="space-y-1.5">
                            <label class="text-[10px] font-black uppercase text-slate-500 tracking-widest ml-1 text-[10px]">Administrative State</label>
                            <select wire:model="stateCode" class="w-full rounded-xl border border-slate-200 bg-slate-50 px-6 py-4 text-[10px] font-black text-slate-900 dark:border-white/5 dark:bg-white/5 dark:text-white uppercase tracking-widest">
                    ...
                    <div class="pt-6 border-t border-slate-100 dark:border-white/5 flex justify-end gap-3">
                         ...
                         <button type="submit" class="rounded-xl bg-slate-900 px-8 py-2.5 text-[11px] font-black uppercase text-white shadow-2xl hover:bg-cyan-600 transition-all active:scale-95">Synchronize Policy Node</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    @if($showDateModal)
        <div class="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <div class="relative w-full max-w-lg rounded-[1.5rem] bg-white shadow-2xl dark:bg-slate-950 border border-slate-200 dark:border-white/10 flex flex-col animate-in fade-in zoom-in duration-200">
                <div class="flex items-center justify-between px-8 py-6 border-b border-slate-50 dark:border-white/5 shrink-0">
                    <h2 class="text-base font-black text-slate-900 dark:text-white uppercase tracking-tight">Inject <span class="text-cyan-500">Temporal Signal</span></h2>
                ...
                <form wire:submit="saveDate" class="p-5 space-y-8">
                     <div class="space-y-1.5">
                        <label class="text-[10px] font-black uppercase text-slate-500 tracking-widest ml-1">Epoch Identifier</label>
                        <input wire:model="dateName" type="text" placeholder="National Celebration..." class="w-full rounded-xl border border-slate-200 bg-slate-50 px-6 py-4 text-[10px] font-black text-slate-900 dark:border-white/5 dark:bg-white/5 dark:text-white uppercase tracking-widest">
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-1.5">
                            <label class="text-[10px] font-black uppercase text-slate-500 tracking-widest ml-1">Temporal Marker (Date)</label>
                            <input wire:model="holidayDate" type="date" class="w-full rounded-xl border border-slate-200 bg-slate-50 px-6 py-4 text-[10px] font-black text-slate-900 dark:border-white/5 dark:bg-white/5 dark:text-white">
                        </div>
                        <div class="space-y-1.5">
                    ...
                    <div class="pt-4 flex justify-end gap-3">
                         ...
                         <button type="submit" class="rounded-xl bg-slate-900 px-6 py-2.5 text-[10px] font-black uppercase text-white shadow-xl hover:bg-cyan-600 transition-all active:scale-95">Lock Marker</button>
                    </div>
                </form>
            </div>
        </div>
