<div class="space-y-5 pb-12 relative">
    ...
    <div class="relative overflow-hidden rounded-xl bg-white/80 px-5 py-4 shadow-sm border border-slate-200 backdrop-blur-xl dark:bg-slate-900/60 dark:border-white/5">
        ...
        <div class="relative flex flex-col items-start justify-between gap-4 lg:flex-row lg:items-center">
            ...
            <div class="flex flex-wrap items-center gap-3">
                {{-- Quick Stats --}}
                <div class="flex items-center gap-4 px-4 border-r border-slate-200 dark:border-white/10 hidden xl:flex">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-2 border-b border-slate-100 dark:border-white/5">
    ...
    <div class="grid grid-cols-1 xl:grid-cols-4 gap-3">
        ...
        <div class="xl:col-span-3 space-y-4">
            @forelse($requests as $req)
                <div wire:click="selectRequest({{ $req->id }})" class="group cursor-pointer relative overflow-hidden rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-white/5 dark:bg-slate-900/60 transition-all hover:shadow-md hover:border-cyan-400/30">
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div class="flex items-center gap-4">
                            {{-- Type Icon (Compact) --}}
                            <div class="h-10 w-10 rounded-xl bg-slate-50 flex items-center justify-center font-black dark:bg-white/5 shadow-inner transition-transform group-hover:scale-105">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <h4 class="text-[10px] font-black text-slate-900 dark:text-white uppercase tracking-tight truncate group-hover:text-cyan-600 transition-colors">{{ $req->title }}</h4>
            ...
            @empty
                <div class="flex flex-col items-center justify-center py-32 text-center rounded-[2rem] border-2 border-dashed border-slate-200 dark:border-white/10">
                    ...
                    <h3 class="text-lg font-black text-slate-900 dark:text-white uppercase tracking-tight">Silent Broadcast</h3>
        ...
        <div class="xl:col-span-1 space-y-6">
            <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-white/5 dark:bg-slate-900/60">
            ...
            <div class="rounded-xl bg-white border border-slate-200 p-5 shadow-sm overflow-hidden relative dark:bg-slate-900/60 dark:border-white/5">
    ...
    @if($showDetailsModal && $selectedRequest)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 lg:p-10">
            ...
            <div class="relative w-full max-w-3xl max-h-[90vh] overflow-hidden rounded-xl bg-white shadow-2xl dark:bg-slate-950 border border-slate-200 dark:border-white/10 flex flex-col animate-in fade-in zoom-in duration-300">
                {{-- Modal Header --}}
                <div class="flex items-center justify-between px-8 py-6 border-b border-slate-100 dark:border-white/5 bg-slate-50/30 dark:bg-white/2 shrink-0">
                    <div class="flex items-center gap-3">
                        <div class="h-10 w-10 rounded-xl bg-white shadow-sm border border-slate-100 flex items-center justify-center dark:bg-white/5 dark:border-white/5">
                ...
                <div class="flex-1 overflow-y-auto px-5 py-4 custom-scrollbar">
                    <div class="grid grid-cols-1 lg:grid-cols-4 gap-3">
                        ...
                        <div class="lg:col-span-3 space-y-8">
                            <div class="rounded-xl border border-slate-100 bg-slate-50/30 p-4 dark:border-white/5 dark:bg-white/2">
                                <h4 class="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400 mb-3">Institutional Objective</h4>
                                <p class="text-[10px] font-bold text-slate-700 dark:text-slate-300 leading-relaxed uppercase tracking-widest">{{ $selectedRequest->description }}</p>
                            </div>
                        ...
                        <div class="lg:col-span-1 space-y-8">
                            ...
                            <div class="rounded-xl p-5 text-center relative overflow-hidden border border-slate-100 bg-white shadow-sm dark:bg-slate-900 dark:border-white/5">
                                    ...
                                    @foreach($selectedRequest->approvals as $idx => $ap)
                                        <div class="relative pl-7">
                                            @if($idx < count($selectedRequest->approvals) - 1)
                                                <div class="absolute left-[13px] top-4 bottom-[-16px] w-[2px] bg-slate-100 dark:bg-white/5"></div>
                                            @endif
                    ...
                    <div class="px-10 py-8 border-t border-slate-50 bg-slate-50/50 dark:border-white/5 dark:bg-white/2 shrink-0">
                        <div class="flex flex-col gap-4">
                            ...
                            <div class="flex items-center justify-end gap-3">
                                ...
                                <button wire:click="approveRequest" class="px-6 py-2 rounded-xl bg-slate-900 text-[10px] font-black uppercase tracking-widest text-white hover:bg-cyan-600 shadow-xl transition-all">Authorize Sequence</button>
                            </div>
                        </div>
                    </div>
                @elseif(Auth::user()->hasAnyRole(['admin', 'hr_manager']) && $selectedRequest->type === 'asset-request' && $selectedRequest->status === 'approved')
                     <div class="px-10 py-8 border-t border-slate-50 bg-cyan-50/30 dark:border-white/5 dark:bg-cyan-500/5 flex items-center justify-between shrink-0">
                        ...
                        <button wire:click="$set('showFulfillmentModal', true)" class="px-6 py-2 rounded-xl bg-cyan-600 text-[10px] font-black uppercase tracking-widest text-white hover:bg-cyan-700 shadow-xl transition-all">Execute Deployment</button>
                    </div>
                @endif
            </div>
        </div>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative w-full max-w-lg rounded-[1.5rem] bg-white shadow-2xl dark:bg-slate-950 border border-slate-200 dark:border-white/10 overflow-hidden animate-in fade-in zoom-in duration-200">
                <div class="border-b border-slate-100 p-5 dark:border-white/5 flex items-center justify-between">
                    <h2 class="text-lg font-black text-slate-900 dark:text-white uppercase tracking-tight">Initiate <span class="text-cyan-500">Protocol</span></h2>
                ...
                <form wire:submit="submitRequest" class="p-5 space-y-8 overflow-y-auto max-h-[70vh] custom-scrollbar">
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-1.5">
                            <label class="text-[9px] font-black uppercase text-slate-500 ml-1 tracking-[0.2em]">Transmission Type</label>
                            <select wire:model.live="requestType" class="w-full rounded-xl border border-slate-200 bg-slate-50 px-5 py-3 text-[10px] font-black text-slate-900 dark:border-white/5 dark:bg-white/5 dark:text-white uppercase tracking-widest">
                        ...
                        <div class="space-y-1.5">
                            ...
                            <div class="relative">
                                ...
                                <input wire:model="requestAmount" type="number" step="0.01" class="w-full rounded-xl border border-slate-200 bg-slate-50 pl-8 pr-5 py-3 text-[10px] font-black text-slate-900 dark:border-white/5 dark:bg-white/5 dark:text-white uppercase tracking-widest">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input wire:model="requestTitle" type="text" placeholder="Short Identifier..." class="w-full rounded-xl border border-slate-200 bg-slate-50 px-5 py-3 text-[10px] font-black text-slate-900 dark:border-white/5 dark:bg-white/5 dark:text-white uppercase tracking-widest">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <textarea wire:model="requestDescription" placeholder="Detailed objective description..." class="w-full rounded-xl border border-slate-200 bg-slate-50 px-5 py-4 text-[10px] font-bold text-slate-900 dark:border-white/5 dark:bg-white/5 dark:text-white uppercase tracking-widest h-32 leading-relaxed"></textarea>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <div 
                            ...
                            class="relative min-h-[120px] w-full rounded-xl border-2 border-dashed border-slate-100 bg-slate-50 flex flex-col items-center justify-center cursor-pointer hover:bg-slate-100 hover:border-cyan-200 transition-all dark:bg-white/5 dark:border-white/5"
                        >
                            ...
                            <div x-show="isUploading" class="absolute inset-0 bg-white/95 dark:bg-slate-950/95 flex flex-col items-center justify-center rounded-xl">
                    ...
                    <div class="pt-6 flex justify-end gap-4">
                        ...
                        <button type="submit" wire:loading.attr="disabled" class="rounded-xl bg-slate-900 px-8 py-2.5 text-[11px] font-black uppercase text-white shadow-2xl hover:bg-cyan-600 transition-all active:scale-95 disabled:opacity-50">Launch Protocol Stream</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <div class="relative w-full max-w-lg rounded-[1.5rem] bg-white shadow-2xl dark:bg-slate-950 border border-slate-200 dark:border-white/10 overflow-hidden animate-in fade-in zoom-in duration-200">
                <div class="border-b border-slate-100 p-5 dark:border-white/5">
                    <h2 class="text-base font-black text-slate-900 dark:text-white uppercase tracking-tight">Deploy <span class="text-cyan-500">Inventory Asset</span></h2>
                ...
                <form wire:submit="fulfillAsset" class="p-5 space-y-6">
                    <div class="space-y-1.5">
                        <label class="text-[9px] font-black uppercase text-slate-500 ml-1 tracking-[0.2em]">Target Resource (Available Inventory)</label>
                        <select wire:model="fulfillmentAssetId" class="w-full rounded-xl border border-slate-200 bg-slate-50 px-5 py-3 text-[10px] font-black text-slate-900 dark:border-white/5 dark:bg-white/5 dark:text-white uppercase tracking-widest">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <textarea wire:model="fulfillmentComment" placeholder="Condition narrative or instructions..." class="w-full rounded-xl border border-slate-200 bg-slate-50 px-5 py-4 text-[10px] font-bold text-slate-900 dark:border-white/5 dark:bg-white/5 dark:text-white uppercase h-24"></textarea>
                    </div>
                    ...
                    <div class="pt-4 flex justify-end gap-4">
                        ...
                        <button type="submit" class="rounded-xl bg-cyan-600 px-6 py-2 text-[10px] font-black uppercase text-white shadow-xl hover:bg-cyan-700 transition-all">Authorize Deployment</button>
                    </div>
                </form>
            </div>
        </div>
